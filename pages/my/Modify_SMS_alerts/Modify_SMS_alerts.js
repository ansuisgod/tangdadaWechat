// pages/my/Modify_SMS_alerts/Modify_SMS_alerts.js
...
Page({
  ...
  onLoad: function (options) {
    ...
    let arrs = JSON.parse(options.arrInfoArr)
    that.setData({
      toId: options.toId,
      arrText: arrs,  //提醒项数组内容
...
* @auther:li
*/
  alarmInfo: function (id) {
    var that = this;
    let platform = abstac.mobilePhoneModels(that.data.platform);//手机型号
    abstac.sms_Interface(app.publicVariable.alarmInfoInfoInterfaceAddress,
      { platform: platform, wx_session_key: session_key, id: id },
      function (res) {//查询成功
        //打印日志
        console.log("****************获取提醒信息某个信息的接口***************");
        console.log(res);
  ...
  sendMsg: function () {
    ...
    if (that.checkPhone(that.data.phone)) { //是手机号 发送验证码
      ...
      abstac.sms_Interface(app.publicVariable.smsInterface, {
        platform: platform, wx_session_key: session_key, phone: that.data.phone, type: '3'
      },
        function (res) { //查询成功
          //打印日志
